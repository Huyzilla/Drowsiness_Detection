...
EYE_AR_THRESH = 0
baseline_head_y = 0;
baseline_head_angle = 0
calibration_y_list = [];
calibration_angle_list = []
...
while cap.isOpened():
    ...
    # TRẠNG THÁI 2: ĐANG HIỆU CHỈNH
        ...
        if mp_results.multi_face_landmarks:
            ...
            if ear_calibrator.update(ear):
                ...
                print(
                    f"--> Nguong EAR: {EYE_AR_THRESH:.3f} | Y ban dau: {baseline_head_y:.2f} | Goc ban dau: {baseline_head_angle:.2f}")
        else:
    ...
    # TRẠNG THÁI 4: ĐANG GIÁM SÁT
        ...
        if mp_results.multi_face_landmarks:
            ...
            face_height = np.linalg.norm(top_head_pt - chin_pt)
            ...
            cv2.line(frame, tuple(top_head_pt), tuple(nose_tip_pt), (0, 255, 0), 2)
            draw_text(frame, f"EAR: {ear:.2f} (N: {EYE_AR_THRESH:.2f})", (w - 280, 30))
            draw_text(frame, f"MAR: {mar:.2f}", (w - 280, 60))
            draw_text(frame, f"Y_Dev_Ratio: {y_deviation / face_height:.2f}", (w - 280, 90))
            draw_text(frame, f"Angle_Dev: {angle_deviation:.2f}", (w - 280, 120))
